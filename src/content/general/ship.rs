...
impl Ship {
    ...
    pub fn from(data: ShipData) -> Result<Self, Error> {
        ...
        let convert_str = |v: Option<String>| -> String {
            v.map(|v| v.clone()).unwrap_or("-".to_owned())
        };
        let convert_value = |v: Option<i32>| -> String {
        ...
        content.push(("Классификационное общество", convert_str(data.classification_society)));
        ...
        content.push(("Заводской номер", convert_str(data.ship_builder_hull_number)));   
