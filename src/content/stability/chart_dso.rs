...
impl ChartDSO {
    ...
    pub fn to_string(self) -> Result<(), Error> {
        ...
        let (header, legend_dso, legend_ddo, legend_h) = if self.language.contains("en") {
            ...
            (
                "Начальная, статическая и динамическая остойчивость",
                "ДСО",
                "ДДО",
                "h",
            )
        ...
        // отрисовка линии ДДО
        chart
            .draw_series(LineSeries::new(self.ddo.clone(), &RGBColor(150, 150, 0)))
            .map_err(|err| error.err(err.to_string()))?
            .label(legend_ddo)
            .legend(|(x, y)| Rectangle::new([(x - 15, y + 1), (x, y)], RGBColor(150, 150, 0)));
        // отрисовка линии h
        chart
            .draw_series(LineSeries::new(self.h.clone(), &RGBColor(150, 0, 0)))
            .map_err(|err| error.err(err.to_string()))?
            .label(legend_h)
        ...
        draw_point(&self.dbg, &mut chart, &[self.h[0]], ShowPoint::X, (0, 5))
            .map_err(|err| error.err(err.to_string()))?;
